...
const navItems: any = {
  ...
  hr: [
    ...
    { title: "Notifications", href: "/hr/notifications", icon: Bell, badgeKey: 'notifications' },
  ...
  candidate: [
    ...
    { title: "Notifications", href: "/candidate/notifications", icon: Bell, badgeKey: 'notifications' }
...
function DashboardLayoutCore({ children, role }: { children: React.ReactNode; role?: string }) {
  ...
  const [badges, setBadges] = useState({ complaints: 0, approvals: 0, notifications: 0, tasks: 0 });
  ...
  useEffect(() => {
    const initApp = async () => {
      if (userProfile) {
        const dbRole = userProfile.role?.toUpperCase() || '';
        ...
        fetchBadgeCounts(userProfile.role, userProfile.id);
        return; 
      }
...
const channel = supabase.channel('global-changes')
  ...
  .on('postgres_changes', { event: '*', table: 'candidate_notifications' }, () => fetchBadgeCounts(currentRole, userProfile?.id))
  .on('postgres_changes', { event: '*', table: 'offer_letters' }, () => fetchBadgeCounts(currentRole, userProfile?.id))
  .on('postgres_changes', { event: '*', table: 'interview_sessions' }, () => fetchBadgeCounts(currentRole, userProfile?.id))
  .subscribe();
  ...
  const fetchBadgeCounts = async (userRole: string, userId?: string) => {
    if (!userId && userProfile?.id) userId = userProfile.id;
    if (!userId) return;
  ...
  const handleLogout = async () => {
    resetClockState();
    setUserProfile(null);
    setBadges({ complaints: 0, approvals: 0, notifications: 0, tasks: 0 });
