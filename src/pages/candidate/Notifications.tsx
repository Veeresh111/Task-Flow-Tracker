...
export default function CandidateNotifications() {
  ...
  const { notifications, unreadCount, markAsRead, markAllAsRead, loading } = useNotifications(userId, "candidate");
  ...
  const getIconForType = (title: string, type?: string) => {
    ...
    if (t.includes('interview') || t.includes('schedule') || t.includes('meeting')) return <Calendar className="w-5 h-5 text-indigo-500" />;
    ...
    if (t.includes('ai') || t.includes('assessment')) return <Sparkles className="w-5 h-5 text-purple-500" />;
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white flex items-center gap-2">
              <Bell className="w-8 h-8 text-blue-600 dark:text-blue-400" /> Notifications
            </h1>
            <p className="text-slate-500 dark:text-slate-400 mt-1">Application status updates, interview invites, and offer alerts.</p>
          </div>
          {unreadCount > 0 && (
            <Button
