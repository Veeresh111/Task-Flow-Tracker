...
export default function Careers() {
  ...
  const handleApplyAction = (job: any) => {
    if (job.apply_link && job.apply_link.startsWith("http")) {
      window.open(job.apply_link, "_blank");
    } else {
      // ✅ FIXED: Routes to the exact form template mapping layout uuid target (`/apply/:formId`) instead of post id
      const targetFormId = job.job_form_id || job.id;
      const cleanTargetRoute = `/apply/${targetFormId}`;
      
      toast({ title: "Opening Form Gateway", description: "Fetching verification fields configuration context..." });
      navigate(cleanTargetRoute);
    }
  };
