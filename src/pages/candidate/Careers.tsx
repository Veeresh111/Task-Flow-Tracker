...
export default function Careers() {
  ...
  useEffect(() => {
    ...
    const jobSub = supabase.channel("candidate-job-feed").on("postgres_changes", { event: "*", schema: "public", table: "job_forms" }, () => fetchVacancies()).subscribe();
  ...
  const fetchVacancies = async () => {
    const { data } = await supabase.from("job_forms").select("id, job_title, department, location, jd_text, requirements, apply_link, requires_assessment").eq("status", "Open").order("created_at", { ascending: false });
                ...
                <Card key={job.id} className="border border-slate-200 bg-white flex flex-col justify-between overflow-hidden shadow-sm transition-all hover:shadow-md hover:border-slate-300">
                  <CardHeader className="p-5 pb-3 space-y-2">
                    <div className="flex justify-between items-start gap-2">
                      <CardTitle className="text-lg font-bold text-slate-900 tracking-tight leading-snug">{job.job_title}</CardTitle>
                    ...
                    <div className="flex flex-wrap gap-3 text-xs font-medium text-slate-500 pt-0.5">
                      ...
                      {job.requires_assessment && <span className="flex items-center gap-1 text-amber-600"><Sparkles className="w-3 h-3" /> Pre-exam Required</span>}
                    </div>
                  </CardHeader>
                  <CardContent className="p-5 pt-0 space-y-4">
                    <div className="space-y-1">
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Description</span>
                      <p className="text-xs text-slate-600 line-clamp-3 leading-relaxed">{job.jd_text}</p>
                    </div>
                    {job.requirements && (
                      <div className="space-y-1">
