...
export default function SmartInbox() {
  ...
  const fetchRealEmails = async () => {
    ...
    try {
      ...
      for (const msg of searchData.messages) {
        ...
        // Step C: Send REAL text to AI via secure proxy
        ...
        try {
          const raw = await callCorporateAI({
            prompt,
            systemInstruction: "You are an enterprise ATS Analyzer. Always output valid JSON.",
          ...
          console.error(`Failed to parse AI response for email ${msg.id}:`, e);
          continue;
        }
  ...
  const pushToDatabase = async () => {
    ...
    try {
       ...
       // Also try to push to recruitment pipeline
       for (const c of realEmails) {
         const extractedEmail = c.from.match(/<(.+)>/)?.[1] || c.from;
         const { data: existingCand } = await supabase.from('candidates').select('id').eq('email', extractedEmail).maybeSingle();
         if (existingCand) {
           await supabase.from('job_applications').upsert({
             ...
             form_id: null,
             status: 'Sourced',
             answers: { source: 'gmail_smart_inbox', subject: c.subject }
           }, { onConflict: 'candidate_id,form_id', ignoreDuplicates: true });
         }
       }
