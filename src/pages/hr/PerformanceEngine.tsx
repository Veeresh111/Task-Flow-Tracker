...
export default function HRPerformanceEngine() {
  ...
  const runGlobalAIEvaluation = async () => {
    ...
    for (let i = 0; i < profiles.length; i++) {
      ...
      const { data: empTasks } = await supabase.from('tasks').select('*').eq('assigned_to', emp.id);
      const completedTasks = empTasks?.filter(t => t.status === 'Completed').length || 0;
      
      const { data: empLogs } = await supabase.from('work_logs').select('*').eq('user_id', emp.id);
      const totalShifts = empLogs?.length || 0;
      ...
      try {
        ...
        Total Tasks Assigned: ${empTasks?.length || 0}
        ...
        const raw = await callCorporateAI({
          prompt,
          systemInstruction: "You are an elite MNC performance assessor. Always output valid JSON.",
        ...
        try {
          ...
          console.error(`Failed to parse AI evaluation for ${emp.name}:`, e);
          continue;
        }
