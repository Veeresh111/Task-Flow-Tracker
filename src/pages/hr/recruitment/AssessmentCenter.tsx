...
export default function AssessmentCenter() {
  ...
  useEffect(() => {
    ...
    const maxViolations = assessmentMeta?.max_violations || 5;
    const handleWindowBlurInfraction = () => {
      proctorLogRef.current.push({
        type: "Tab Switch / Window Focus Lost",
      ...
      setViolationCount(prev => {
        ...
        toast({
          ...
          description: `Window focus lost. Violation register: ${nextCount}/${maxViolations}. Reaching ${maxViolations} results in immediate failure status.`,
          variant: "destructive"
        });
  ...
  const generateAssessment = async () => {
    ...
    setAiLoading(true);
    try {
      const prompt = `Generate a professional technical test assessment for the role: "${assessmentTitle}". 
JD Context: "${jdText}". Difficulty: "${assessmentDifficulty}". Create exactly ${questionCount} multiple-choice questions.
Output STRICTLY a valid JSON array where each item has: {"question": "...", "options": ["A", "B", "C", "D"], "correctAnswer": "A"}.`;
      ...
      setAssessmentQuestions(parsed);
      toast({ title: "Questions Generated", description: `${parsed.length} questions created. Review and edit if needed.` });
    } catch (e: any) {
      toast({ title: "Generation Error", description: "AI question generation failed. Use the manual editor below.", variant: "destructive" });
    } finally {
      setAiLoading(false);
    }
  };
    ...
    try {
      const { data, error } = await supabase
        ...
        .insert([{
          ...
          duration_minutes: Math.max(2, Math.floor(Number(durationMinutes) || 60)),
            ...
            <Card className="max-w-xl mx-auto border-slate-200 bg-white shadow-xl rounded-xl overflow-hidden">
              ...
              <CardContent className="p-6 space-y-5 text-xs font-medium text-slate-600 leading-relaxed">
                ...
                <div className="bg-slate-50 border p-4 rounded-xl space-y-2 border-slate-200 text-slate-600">
                  ...
                  <p>• Logging {assessmentMeta?.max_violations || 5} infraction violations results in automatic script locking and an absolute fail status.</p>
            ...
            <div className="grid md:grid-cols-4 gap-6 items-start">
              <div className="md:col-span-3 space-y-4">
                <div className="bg-white border p-4 rounded-xl flex items-center justify-between shadow-sm">
                  ...
                  <span className="text-[10px] uppercase font-black px-2.5 py-1 rounded-full border bg-red-50 text-red-700 border-red-200 tracking-wider animate-pulse">Infractions: {violationCount} / {assessmentMeta?.max_violations || 5}</span>
                </div>
