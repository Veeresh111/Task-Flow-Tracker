...
export default function TeamLeadAnalytics() {
  ...
  const fetchTeamAnalytics = async () => {
    ...
    const [profilesRes, tasksRes, logsRes, teamMetricsRes] = await Promise.all([
      supabase.from('profiles').select('*'),
      supabase.from('tasks').select('*'),
      supabase.from('work_logs').select('*'),
      supabase.rpc('get_team_metrics', { lead_id: user.id })
    ]);

    const allProfiles = profilesRes.data || [];
    const tasks = tasksRes.data || [];
    const logs = logsRes.data || [];
    const teamMetrics = teamMetricsRes.data;
        ...
        {loading ? <div className="flex justify-center p-20"><Loader2 className="w-10 h-10 animate-spin text-blue-600" /></div> : (
          <>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <Card className="shadow-sm border-slate-200"><CardContent className="p-5 flex items-center gap-4"><div className="p-3 bg-slate-100 text-slate-600 rounded-full"><Users className="w-5 h-5"/></div><div><p className="text-xs font-bold text-slate-500 uppercase">My Team Size</p><h2 className="text-2xl font-black text-slate-800">{teamMetrics?.team_size ?? employees.length}</h2></div></CardContent></Card>
