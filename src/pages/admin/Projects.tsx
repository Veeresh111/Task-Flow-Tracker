...
export default function AdminProjects() {
  ...
  const saveProjectChanges = async (e: React.FormEvent) => {
    ...
    if (editingProject) {
      ...
      if (error) toast({ title: "Update Failed", description: "The project could not be updated. Please try again.", variant: "destructive" });
      else toast({ title: "Project Updated", description: "The project has been updated successfully." });
    } else {
      // CREATE NEW
      ...
      if (error) toast({ title: "Creation Failed", description: "The project could not be created. Please try again.", variant: "destructive" });
      else toast({ title: "Project Created", description: "The new project has been created successfully." });
    }
  ...
  const deleteProject = async () => {
    if (!editingProject || !window.confirm("Confirm Deletion\n\nAre you sure you want to delete this project?")) return;
    setLoading(true);
    const { error } = await supabase.from('projects').delete().eq('id', editingProject.id);
    
    if (error) toast({ title: "Deletion Failed", description: "The project could not be deleted. Please try again.", variant: "destructive" });
    else toast({ title: "Project Deleted", description: "The project has been deleted successfully." });
  ...
  const generateProjectRoadmap = async (proj: any) => {
    setIsAiLoading(true);
    toast({ title: "AI Analysis Started", description: "Analyzing team structure and project requirements..." });
