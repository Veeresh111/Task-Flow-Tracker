...
export default function AdminNotifications() {
  ...
  const { notifications, unreadCount, markAsRead, markAllAsRead, loading } = useNotifications(userId, "admin");
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white flex items-center gap-2">
              <Bell className="w-8 h-8 text-blue-600 dark:text-blue-400" /> Admin Activity Hub
            </h1>
            <p className="text-slate-500 dark:text-slate-400 mt-1">Review system alerts, administrative tasks, and operational updates.</p>
          </div>
          {unreadCount > 0 && (
            <Button
