...
Do not wrap the output in markdown block headers or backticks.`;
      ...
      const content = await callCorporateAI({ prompt, temperature: 0.1 });
      ...
      toast({ title: "Analysis Complete", description: "Performance analysis has been saved successfully." });
    } catch (err: any) {
      console.error(err);
      toast({ title: "Analysis Failed", description: "Could not complete the performance analysis. Please try again.", variant: "destructive" });
    } finally {
              ...
              {loading ? (
                ...
                <TableRow key={profile.id} className="hover:bg-slate-50/40 transition-colors">
                  ...
                  <TableCell className="text-xs font-medium text-slate-500">
                    <span className="flex items-center gap-1"><Calendar className="w-3.5 h-3.5 text-slate-400"/> {profile.created_at ? new Date(profile.created_at).toLocaleDateString('en-IN', {day: 'numeric', month: 'short', year: 'numeric'}) : "N/A"}</span>
                  </TableCell>
