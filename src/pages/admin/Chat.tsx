...
export default function UniversalSmartChat() {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('office-comms-live')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages' }, async (payload) => {
        ...
        console.debug("Chat message routed");
        ...
        if (interactingPeerId) {
          setUsers(prevUsers => {
            ...
            console.debug("User list reordered");
  ...
  const initChat = async () => {
    ...
    try {
      ...
      console.error(err);
      toast({ title: "Connection Error", description: "Unable to load messages. Please try refreshing the page.", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    // DIAGNOSTIC PRINTS FOR TRANSACTION AUDITING
        console.debug("Sender verified:", verifiedAuthUserId);
    ...
    try {
      if (activeChat === "BROADCAST") {
        ...
        if (!holdsManagementClearance) {
          toast({
            ...
            description: "You are not authorized to send broadcast messages.",
            variant: "destructive"
          });
          return;
        }
      ...
      toast({ title: "Send Failed", description: "Your message could not be sent. Please try again.", variant: "destructive" });
    }
  };
  ...
  const deleteMessage = async (messageId: string) => {
    try {
      ...
      toast({ title: "Delete Failed", description: "The message could not be deleted. Please try again.", variant: "destructive" });
    }
  };
