...
export default function AdminPerformanceEngine() {
  ...
  const runGlobalAIEvaluation = async () => {
    setLoading(true);
    toast({ title: "Evaluation Started", description: "Analyzing tasks, hours, and behaviors across the organization..." });
...
Output STRICTLY JSON. Format: {"score": 85, "reason": "Consistent task completion..."}`;
        ...
        const result = await callCorporateAI({ prompt, temperature: 0.1 });
    ...
    toast({ title: "Evaluation Complete", description: "All employee ratings have been updated successfully." });
    fetchDirectory();
    setLoading(false);
  };
