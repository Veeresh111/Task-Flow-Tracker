...
async function main() {
  ...
  let candidate_name = "Schema Probe";
  let candidate_email = "probe@test.com";
  let job_title = "Engineer";
  let status = "Pending Approval";

  // Insert with all discovered required cols
  const { data: ins, error: e } = await admin
    .from("offer_letters")
    .insert({ candidate_name, candidate_email, job_title, status })
    .select("*")
    .single();
  if (e) {
