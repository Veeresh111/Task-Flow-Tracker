...
export default function TeamLeadTasks() {
  ...
  // === IMPROVED QWEN 3 AI TASK DELEGATOR ===
  const generateTaskPlan = async (task: any) => {
    ...
    try {
      ...
      // Debug token
      console.log("🔑 VITE_HF_TOKEN loaded:", !!HF_TOKEN);
      if (!HF_TOKEN) {
        throw new Error("HF_TOKEN is missing. Check your .env file and restart dev server.");
      }
...
Keep response concise and professional.`;
      ...
      if (!response.ok) {
        ...
        throw new Error(`Qwen API Error ${response.status}: ${errorText.substring(0, 100)}`);
      }

      const data = await response.json();
      const aiResponse = data.choices?.[0]?.message?.content?.trim() 
        || "Qwen 3 returned an empty response.";

      setAiTaskPlan(aiResponse);
      toast({ title: "✅ Qwen 3 Analysis Complete" });
