...
export default function HRPayroll() {
  ...
  const fetchPayrollDatabase = async () => {
    ...
    try {
      ...
      if (profilesResponse.error && (profilesResponse.error.code === 'PGRST204' || profilesResponse.error.message?.includes('employment_status'))) {
        profilesResponse = await supabase.from('profiles').select('*');
      }
      ...
      if (profiles) {
        ...
        const enrichedData = profiles.filter(emp => emp.role === 'employee' || emp.id === user?.id).map(emp => {
          ...
          const record = {
            ...
            performanceData: [
              { month: 'Jan', performance: emp.performance_score ? Math.max(40, emp.performance_score - 12) : 72, morale: 80 },
              { month: 'Feb', performance: emp.performance_score ? Math.max(45, emp.performance_score - 6) : 76, morale: 81 },
              { month: 'Mar', performance: emp.performance_score || 80, morale: 84 },
              { month: 'Apr', performance: emp.performance_score || 84, morale: 85 },
              { month: 'May', performance: emp.performance_score || 87, morale: 88 },
              { month: 'Jun', performance: emp.performance_score || 92, morale: 90 },
            ]
          };
      ...
      toast({
        ...
        description: e.message || "Failed to load database ledger nodes.",
        variant: "destructive"
      });
    } finally {
      setLoading(false);
  ...
  const fetchPersonalHistoricalSlips = async (empId: string, currentRecord: any) => {
    try {
      ...
      if (!data || data.length === 0) {
        setHistoricalSlips([
          { payout_month: 'May 2026', net_paid: currentRecord.netPay, base_salary: currentRecord.base, basicPay: currentRecord.basicPay, hra: currentRecord.hra, ta: currentRecord.ta, da: currentRecord.da, variablePay: currentRecord.variablePay, tax_deducted: currentRecord.tax, pf_contribution: currentRecord.providentFund, name: currentRecord.name, department: currentRecord.department, email: currentRecord.email, tenure: currentRecord.tenure },
          { payout_month: 'April 2026', net_paid: currentRecord.netPay, base_salary: currentRecord.base, basicPay: currentRecord.basicPay, hra: currentRecord.hra, ta: currentRecord.ta, da: currentRecord.da, variablePay: currentRecord.variablePay, tax_deducted: currentRecord.tax, pf_contribution: currentRecord.providentFund, name: currentRecord.name, department: currentRecord.department, email: currentRecord.email, tenure: currentRecord.tenure },
          { payout_month: 'March 2026', net_paid: currentRecord.netPay, base_salary: currentRecord.base, basicPay: currentRecord.basicPay, hra: currentRecord.hra, ta: currentRecord.ta, da: currentRecord.da, variablePay: currentRecord.variablePay, tax_deducted: currentRecord.tax, pf_contribution: currentRecord.providentFund, name: currentRecord.name, department: currentRecord.department, email: currentRecord.email, tenure: currentRecord.tenure }
        ]);
      } else {
  ...
  const handleCommitModulatedCompensation = async () => {
    ...
    try {
      ...
      toast({ 
        title: `Compensation Structured`, 
        description: `Successfully balanced payroll parameters to a new dynamic CTC of ₹${updatedGrossCTCValue.toLocaleString()}.` 
      });
      ...
      toast({ title: "Modulation Denied", description: err.message, variant: "destructive" });
    }
  };
  ...
  const handleProcessSeparation = async () => {
    ...
    try {
      ...
      toast({ title: "Offboarding Blocked", description: err.message, variant: "destructive" });
    } finally {
