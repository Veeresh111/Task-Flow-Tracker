...
import { Loader2, UserPlus, Sparkles, FileText, Briefcase, Mail, UploadCloud, ClipboardPaste, CheckCircle2, XCircle, Users, Download, Video, BrainCircuit, Play, ExternalLink, Share2, FormInput, FileQuestion, Edit3, Send, X } from "lucide-react";
...
export default function HRRecruitment() {
  ...
  const callGemini = async (prompt: string, sysInstruct?: string) => {
    ...
    if (sysInstruct) modelConfig.systemInstruction = sysInstruct;
  ...
  const generateSmartFormSchema = async () => {
    ...
    try {
      ...
      CRITICAL INSTRUCTION: These 3 technical questions MUST be complete, grammatically perfect, full-sentence questions. They must be highly systematic and easy for a candidate to understand. Do not use fragments.
      ...
      [
        ...
        {"id": "notice_period", "label": "How soon can you start working?", "type": "select", "options": ["Immediate", "15 Days"], "required": true}
      ]`;
  ...
  // --- SMART EMAIL COMMUNICATION SYSTEM ---
  const draftShortlistEmail = async () => {
    ...
    try {
      ...
      const emailText = await callGemini(prompt);
  ...
  // --- CORPORATE FIX: APTITUDE TEST BUILDER WITH EXPONENTIAL RETRY (429 FIX) ---
  const generateAssessment = async () => {
    ...
    // Exponential Backoff Loop to perfectly bypass Google 429 Quota Exceeded Limits
      ...
      try {
        ...
        if (errMsg.includes('429') || errMsg.includes('Quota')) {
          if (attempt >= maxRetries) {
            toast({ title: "API Quota Exceeded", description: "Google rate limit reached. Please wait 60 seconds before generating.", variant: "destructive" });
          } else {
            toast({ title: "Network Busy (429)", description: `API limits hit. Auto-retrying in 5 seconds... (Attempt ${attempt}/${maxRetries})` });
            await sleep(5000 * attempt); // Dynamic wait to let Google servers cool down
          }
        } else {
          toast({ title: "Generation Failed", description: "Could not compile assessment. Please verify your JD format.", variant: "destructive" });
          break; // Break instantly if it's not a quota error
        }
      }
    }
  ...
  const startLiveInterview = async () => {
    if (!jdRole) return toast({ title: "JD Required", variant: "destructive" });
    setInterviewState('active');
    setAiLoading(true);
    try {
      const prompt = `Act as an HR Copilot. For a "${jdRole}" interview, give exactly 4 specific interview questions formatted as a raw JSON array of strings. No markdown. Example: ["Q1?", "Q2?"]`;
      const result = await callGemini(prompt);
      let cleanText = result.replace(/```[a-z]*\n?/gi, '').replace(/```/g, '').trim();
      const startIdx = cleanText.indexOf('[');
      const endIdx = cleanText.lastIndexOf(']');
  ...
  const endInterview = async () => {
    setAiLoading(true);
    try {
      const prompt = `Act as an AI Interview Analyzer for "${jdRole}". Interview finished. Generate strict JSON. Format: {"finalScore": 82, "report": "Candidate showed strong leadership.", "analytics": [{"name": "Tech", "value": 70}]}`;
      const result = await callGemini(prompt);
      let cleanText = result.replace(/```[a-z]*\n?/gi, '').replace(/```/g, '').trim();
      const startIdx = cleanText.indexOf('{');
      const endIdx = cleanText.lastIndexOf('}');
  ...
  const generateJD = async () => {
    ...
    setAiLoading(true);
    try {
      const prompt = `Write a highly professional, 3-paragraph Job Description for a "${jdRole}" at FWC India. Include Requirements and Responsibilities. No markdown.`;
      setGeneratedJD(await callGemini(prompt));
    } catch (e) { toast({title:"Error", variant:"destructive"}); }
    setAiLoading(false);
  };

  const generateOffer = async () => {
    if (!offerName || !offerSalary || !jdRole) return toast({ title: "Fields Required", variant: "destructive" });
    setAiLoading(true);
    try {
      const prompt = `Draft a formal corporate offer letter for candidate "${offerName}" offering a salary of INR ${offerSalary} for position "${jdRole}". Professional tone. No markdown.`;
      setGeneratedOffer(await callGemini(prompt));
    } catch (e) { toast({title:"Error", variant:"destructive"}); }
    setAiLoading(false);
  };
  ...
  return (
    <DashboardLayout role="hr">
      <div className="max-w-7xl mx-auto space-y-6 animate-fade-in pb-12">
        ...
        {/* --- ASSESSMENT BUILDER TAB (WITH 429 RETRY & MANUAL EDIT OPTION) --- */}
            ...
            <div className="md:col-span-1 space-y-4">
              <Card className="shadow-sm border-slate-200"><CardHeader className="bg-slate-50 border-b pb-4"><CardTitle className="text-sm font-bold flex items-center gap-2"><Video className="w-4 h-4 text-blue-600"/> Room Config</CardTitle></CardHeader><CardContent className="p-4 space-y-4">
                <div className="space-y-2">
                  ...
                  <Input placeholder="E.g. Senior Backend Dev" value={jdRole} onChange={e=>setJdRole(e.target.value)} disabled={interviewState!=='idle'} />
                </div>
                <div className="space-y-2">
        ...
        {activeTab === 'jd' && (
          <div className="grid md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <Input placeholder="Role Title (e.g., Cloud Architect)" value={jdRole} onChange={e=>setJdRole(e.target.value)}/>
              <Button onClick={generateJD} disabled={aiLoading} className="w-full bg-indigo-600 text-white">{aiLoading?<Loader2 className="animate-spin w-4 h-4"/>:<Sparkles className="w-4 h-4 mr-2"/>} Draft JD</Button>
            </div>
            <Card className="bg-slate-50"><CardContent className="p-6 whitespace-pre-wrap text-sm text-slate-700">
              {generatedJD || "AI Drafted JD will appear here..."}
            </CardContent></Card>
          </div>
        )}
        ...
        {activeTab === 'offer' && (
          <div className="grid md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <Input placeholder="Job Title Context (e.g., Cloud Architect)" value={jdRole} onChange={e=>setJdRole(e.target.value)}/>
              <Input placeholder="Candidate Name" value={offerName} onChange={e=>setOfferName(e.target.value)}/>
              <Input placeholder="Total CTC (e.g., 18,00,000)" value={offerSalary} onChange={e=>setOfferSalary(e.target.value)}/>
              <Button onClick={generateOffer} disabled={aiLoading} className="w-full bg-indigo-600 text-white">{aiLoading?<Loader2 className="animate-spin w-4 h-4"/>:<Sparkles className="w-4 h-4 mr-2"/>} Generate Letter</Button>
            </div>
            <Card className="bg-slate-50"><CardContent className="p-6 whitespace-pre-wrap text-sm text-slate-700">
              {generatedOffer || "AI Generated Offer Letter will appear here..."}
            </CardContent></Card>
          </div>
        )}
      </div>
