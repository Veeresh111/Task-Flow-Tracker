...
export default function CandidateMessages() {
  ...
  useEffect(() => {
    ...
    const jobChannel = supabase.channel("hr-broadcast-jobs")
      .on("postgres_changes", { event: "*", schema: "public", table: "recruitment_posts" }, () => fetchVacancies())
      .subscribe();
  ...
  const fetchVacancies = async () => {
    try {
      ...
      const { data, error } = await supabase.from("recruitment_posts").select("*").eq("status", "Open").order("created_at", { ascending: false });
      if (error) throw error;
      setVacancies(data || []);
    } catch (err) {
      console.error(err);
  ...
  const handlePublishJob = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from("recruitment_posts").insert([{
        ...
        assessment_required: jobForm.assessmentRequired,
                  ...
                  {vacancies.map((job) => (
                    <Card key={job.id} className="border border-slate-200 bg-white flex flex-col justify-between overflow-hidden shadow-sm hover:border-blue-200 transition-all">
                      ...
                      <div className="p-3 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
                        <span className="text-[10px] text-slate-400 font-semibold uppercase">{job.assessment_required ? "⚡ Requires Exam" : "📄 Review Only"}</span>
