...
export default function EmployeeWorkLogs() {
  ...
  const calculateHours = (start: string | null, end: string | null) => {
    ...
    try {
      ...
      if (end) {
        ...
        // If they forgot to log out, cap it at 11:59 PM of that day
        endTime = now.getTime() > eod.getTime() ? eod.getTime() : now.getTime();
      }

      const hours = (endTime - startTime) / 3600000;
      return hours > 24 ? 24 : (hours > 0 ? hours : 0); // Hard cap at 24 hrs
    } catch { return 0; }
  };
  ...
  workLogs.forEach(log => {
    ...
    if (!dailySummaries[dateStr]) {
      dailySummaries[dateStr] = { date: dateStr, totalHours: 0, status: 'Completed', sortDate: startMs, firstIn: startMs, lastOut: endMs, location: log.work_location || 'WFO' };
    }
    ...
    <DashboardLayout role="employee">
      <div className="max-w-6xl mx-auto space-y-6 animate-fade-in pb-12">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-xl shadow-sm border border-slate-200">
          <div><h1 className="text-3xl font-bold tracking-tight text-slate-900 flex items-center gap-2"><Calendar className="w-8 h-8 text-blue-600" /> My Timesheet</h1><p className="text-slate-500 mt-1">Your official daily attendance and work hours record.</p></div>
              ...
              <div className="overflow-x-auto">
                <Table>
                  ...
                  <TableBody>
                    ...
                    {aggregatedData.length === 0 && <TableRow><TableCell colSpan={6} className="text-center p-8 text-slate-500">No time recorded yet.</TableCell></TableRow>}
                  </TableBody>
                </Table>
              </div>
            </CardContent>
