...
export default function AdminAnalytics() {
  ...
  const handleFetchLatestAIAnalysis = async () => {
    ...
    try {
      ...
      if (data) {
        ...
        toast({
          title: "Analysis Registry Synchronized",
          description: "Successfully reloaded the latest cryptographic AI report from production tables."
        });
      } else {
        toast({
          title: "Registry Empty",
          description: "No historical AI reports found inside the reports table database nodes.",
          variant: "destructive"
        });
      }
    } catch (err: any) {
  ...
  const initiateMeeting = async (empName: string, empId: string) => {
    try {
      ...
      if (error) throw error;
      toast({ title: "Meeting Initiated", description: `A high-priority sync task has been committed to the ledger for ${empName}.` });
    } catch (err: any) {
      toast({ title: "Task Creation Aborted", description: `${err.message} - Ensure your authenticated role has direct GRANT privileges on public.tasks table inside Supabase.`, variant: "destructive" });
    }
  };
    ...
    try {
      const safeFetch = async (queryPromise: Promise<any>, tableName: string, fallbackData: any = []) => {
        try {
          ...
          if (result.error) {
            toast({
              title: `Data Source Warning: [${tableName}]`,
              description: `${result.error.message} (Code: ${result.error.code || 'UNKNOWN'}). Using local workspace default arrays.`,
              variant: "destructive"
            });
            return fallbackData;
          }
      ...
      const deptCounts = profiles.filter(p => p.role === 'employee' && p.employment_status !== 'terminated').reduce((acc: any, p: any) => {
        const d = p.department || 'General Operations';
        acc[d] = (acc[d] || 0) + 1;
        return acc;
      }, {});
      ...
      const compiledAnalytics = profiles.filter(p => p.employment_status !== 'terminated').map(emp => {
        ...
        const livePerformanceMetricScore = empAnalyticsRow ? Number(empAnalyticsRow.ai_performance_score) : (Number(emp.performance_score) || Math.round(completionRate));
      ...
      const sourcingCount = onboardings.filter(o => o.onboarding_stage === 'sourcing').length;
      const screeningCount = onboardings.filter(o => o.onboarding_stage === 'screening').length;
      const interviewCount = onboardings.filter(o => o.onboarding_stage === 'interview').length;
      const offeredCount = onboardings.filter(o => o.onboarding_stage === 'pending').length;
      ...
      console.log("Attendance Data", dbAttendanceRecords);
      ...
      console.log("Trend Data", performanceTrends);
      ...
      toast({ title: "Aggregation Stalled", description: "Internal system node query error.", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleNewReport = async () => {
    ...
    try {
      ...
      if (error) {
        toast({
          title: "Report Mutation Interrupted",
          description: `${error.message} (Code: ${error.code}). Run audited SQL Grants script inside your dashboard panel to authorize write access.`,
          variant: "destructive"
        });
        return;
      }
      ...
      toast({ title: "Smart Report Audited", description: "Company data matrices parsed and saved with AI insights summary." });
    } catch (e: any) {
      console.error(e);
      toast({ title: "Report Execution Interrupted", description: e.message, variant: "destructive" });
    } finally {
