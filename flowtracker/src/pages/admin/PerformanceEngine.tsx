...
export default function AdminPerformanceEngine() {
  ...
  const runGlobalAIEvaluation = async () => {
    ...
    // Pull from local variables or fallback safely to corporate token reference
    ...
    for (let i = 0; i < profiles.length; i++) {
      ...
      try {
        ...
        const response = await fetch("https://router.huggingface.co/v1/chat/completions", {
          ...
          body: JSON.stringify({
            ...
            temperature: 0.1 // Low temperature level guarantees strict adherence to the requested JSON layout schema
          })
        });
        ...
        if (!response.ok) {
          throw new Error(data?.error?.message || "Failed to process target system node pipeline.");
        }

        let cleanText = data.choices[0].message.content || "";
        
        // SYSTEMATIC CLEANING PIPELINE: Eradicates reasoning blocks and text formatting wrappers cleanly
        cleanText = cleanText
      ...
      if (i < profiles.length - 1) await sleep(4000); // API Limit Protection throttling
    }
